        ...
        <div className="space-y-6">
          {orders.map((order) => (
            <div key={order._id} className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-3 mb-4">
                {order.items.map((item) => {
                  const menuItem = menuItems.get(item.menuItem);
                  if (!menuItem) return null; // If the menu item is not found, skip

                  return (
                    <div key={item.menuItem} className="flex justify-between items-center">
                      <div className="flex items-center gap-4">
                        <img
                          src={menuItem.image} // Directly use the image URL from backend
                          alt={menuItem.name}
                          className="w-16 h-16 object-cover rounded-md"
                        />
                        <div>
                      ...
                      <span className="font-medium">
                        {formatPrice(item.quantity * menuItem.price)} {/* Adjust price dynamically */}
                      </span>
                    </div>
                  );
                })}
